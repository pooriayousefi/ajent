...
namespace pooriayousefi
{
    ...
    inline JSON make_error(const std::string &message) { return {{"error", message}, {"is_error", true}}; }
    inline JSON make_result(JSON value) { return {{"result", std::move(value)}, {"is_error", false}}; }
    ...
    JSON tool_create_directory(const std::string &path)
    {
        ...
        // FIX: Replaced unconditional overwrite with proper else block
    ...
    {
        std::error_code ec;
        if (!fs::is_directory(path, ec) || ec) return make_error("Directory not found: " + path);
        
        JSON content = JSON::array();
        fs::directory_iterator it(path, fs::directory_options::skip_permission_denied, ec);
        if (ec) return make_error("Failed to open directory: " + ec.message());
        ...
        for (const auto& entry : it)
        {
            ...
            if (!is_dir) size = entry.file_size(entry_ec);
    ...
    JSON tool_iterate_directories_recursively(const std::string &path)
    {
        ...
        // FIX: Refactored to cleanly bundle result
        JSON payload;
    ...
    {
        std::error_code ec;
        if (!fs::exists(path, ec)) return make_error("Directory not found: " + path);
        
        auto removed_count = fs::remove_all(path, ec);
        if (ec) return make_error("Failed to delete directory: " + ec.message());
    ...
    JSON tool_get_last_modified_file(const std::string &path)
    {
        ...
        if (!found) return make_error("No files found in directory: " + path);
    ...
    JSON get_schema()
    {
        return JSON::parse(R"(
            [
                {"type": "function", "function": {"name": "create_directory", "description": "Creates a directory at the given absolute path. If it already exists, does nothing.", "parameters": {"type": "object", "properties": {"path": {"type": "string", "description": "The absolute path of the directory to create."}}, "required": ["path"]}}},
                {"type": "function", "function": {"name": "list_directory", "description": "Lists the immediate files and folders inside a given directory (non-recursive).", "parameters": {"type": "object", "properties": {"path": {"type": "string", "description": "The absolute path of the directory to list."}}, "required": ["path"]}}},
                {"type": "function", "function": {"name": "iterate_directories_recursively", "description": "Iterates a given directory recursively and returns a flat list of all files and folders inside it.", "parameters": {"type": "object", "properties": {"path": {"type": "string", "description": "The absolute path of the directory which has to be iterated."}}, "required": ["path"]}}},
                {"type": "function", "function": {"name": "delete_directory", "description": "Deletes a directory and all of its contents recursively. Use with caution.", "parameters": {"type": "object", "properties": {"path": {"type": "string", "description": "The absolute path of the directory to delete."}}, "required": ["path"]}}},
                {"type": "function", "function": {"name": "get_last_modified_file", "description": "Finds and returns the absolute path of the most recently modified file in a given directory.", "parameters": {"type": "object", "properties": {"path": {"type": "string", "description": "The absolute path of the directory to search."}}, "required": ["path"]}}}
            ]
        )");
    }
...
int main(int argc, char* argv[])
{
    ...
    try 
    {
        if (argc != 3) throw std::invalid_argument("Usage: directory_server <host> <port>");
        host = argv[1];
        ...
        svr.Post("/tools/call", [](const httplib::Request &req, httplib::Response &res) {
            ...
            res.set_content(pooriayousefi::handle_tool_call(arguments, tool_name).dump(), "application/json");
        });
